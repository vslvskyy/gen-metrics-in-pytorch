...
class ImprovedPRD(BaseGanMetricStats):
    """
    Class for Improved PRD (Precision and Recall for Distributions) calculation
    """
    ...
    def get_knn_distances(
        ...
        for i, f in enumerate(features):
            # pairwise_distances = np.zeros(shape=(len(features)))

            # for i1, f1 in enumerate(features):
            #     d = torch.norm(f - f1, 2)
            #     pairwise_distances[i1] = d

            pairwise_distances = torch.norm(features - f, 2, dim=-1)

            kth_distances[i] = np.partition(pairwise_distances.cpu().numpy(), self.k)[self.k]
    ...
    def calc_coverage(
        ...
        #     for i, a in enumerate(a_ftrs):
        #         d = torch.norm(b - a, 2)
        #         if d <= a_knn_distances[i]:
        #             cnt += 1
        #             break
    ...
    def calc_metric(
        ...
        if isinstance(generated_data, DataLoader):
            ...
            raise TypeError("ImprovedPRD.calc_metric: generated_data should be DataLoader")
        ...
        with Timer(time_info, "ImprovedPRD: calc_metric"):
            if self.real_knn_distances is None:
                ...
                if self.real_ftrs is None:
                    with Timer(time_info, "InceptionV3: get_features (real)"):
                        self.real_ftrs = self.inception_model.get_features(self.real_data_loader, dim=2048, device=self.device)
            ...
            with Timer(time_info, "InceptionV3: get_features (generated)"):
                generated_ftrs = self.inception_model.get_features(generated_data_loader, dim=2048, device=self.device)

            num_obj = min(len(self.real_ftrs), len(generated_ftrs))  # to be sure, that |real_ftrs| = |generated_ftrs|
            if num_obj <= 0:
                raise TypeError("ImprovedPRD.calc_metric: generated or real data manifold is empty")

            with Timer(time_info, "ImprovedPRD: get_knn_distances (generated)"):
                self.generated_knn_distances = self.get_knn_distances(generated_ftrs[:num_obj])

            if precision:
                with Timer(time_info, "ImprovedPRD: calc_coverage (precision)"):
                    precision = self.calc_coverage(
                        # {key: val for key, val in self.real_knn_distances.items() if key < num_obj},
        ...
        if precision and recall: return (precision, recall), time_info
        if precision: return precision, time_info
        if recall: return recall, time_info
